...
function SWEP:HandleRecoil()
	...
	if CLIENT then
		...
		ang.p = ang.p - self.Primary.RecoilAdd * Crouched * Moving * math.Remap(self.Owner:Aim(), 0, 100, 1, 0.5)
		...
		self.Owner:ViewPunch(Angle(-self.Primary.RecoilAdd, self.Primary.RecoilAdd * math.Rand(-0.33 * Irons, 0.33 * Irons), 0) * Crouched * Moving * math.Remap(self.Owner:Aim(), 0, 100, 1, 0.5))
	...
	elseif self.Melee then
		...
		if tr.Hit then
			...
			if SERVER then
				...
				if not self.Owner.NextStrength then self.Owner.NextStrength = 0 end
