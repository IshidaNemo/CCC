...
SWEP.Itemize = true;
SWEP.ItemDescription = "Scanners are equipped with a searchlight and a camera used to take photographic images. The flash emitted when taking an image can temporarily blind a person, even when not looking directly at the flash.";
SWEP.ItemWeight = 2;
SWEP.ItemFOV = 11;
...
function SWEP:PrimaryUnholstered()
	...
	if( SERVER ) then
		...
		--ent:SetKeyValue( "OnlyInspectPlayers", 1 );
		ent:Spawn();
		ent:Activate();
		ent:Fire( "Kill", "", 1200 );
